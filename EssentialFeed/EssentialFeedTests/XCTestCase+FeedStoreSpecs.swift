...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func expect(_ sut: FeedStore, toRetrieve expectedResult: FeedStore.RetrievalResult, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.retrieve { receivedResult in
            ...
            case let (.success(receivedCachedFeed), .success(expectedCachedFeed)):
                XCTAssertEqual(receivedCachedFeed?.feed, expectedCachedFeed?.feed, file: file, line: line)
                XCTAssertEqual(receivedCachedFeed?.timestamp, expectedCachedFeed?.timestamp, file: file, line: line)
            default:
    ...
    func insert(_ cache: (feed: [LocalFeedImage], timestamp: Date), to sut: FeedStore, file: StaticString = #filePath, line: UInt = #line) -> Error? {
        ...
        sut.insert(cache.feed, timestamp: cache.timestamp) { insertResult in
            if case let .failure(receivedInsertionError) = insertResult {
